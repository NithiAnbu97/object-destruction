...
const scores = [
  ...
  //  "Debra Beard", "Nettie Hancock",  "Hatfield Hod
  
  const ispass = scores.filter(score => score.marks >= 40);
const isfail = scores.filter(score => score.marks < 40).map(scores=>scores.name);
 const grades = scores.map(function(entry)
 {
  if(entry.marks<=40)
  entry.grade='fail';
  else if(entry.marks>=40)
  entry.grade='pass';
